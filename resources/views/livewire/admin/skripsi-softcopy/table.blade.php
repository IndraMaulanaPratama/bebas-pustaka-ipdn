                    ...
                    <tbody>
                        @foreach ($data as $item)
                            @php
                                ...
                                $pivot = PivotSkripsi::where('PIVOT_PRAJA', $item->SKRIPSI_PRAJA)->first();
                                // dd($pivot->donasi_elektronik->ELEKTRONIK_STATUS);

                                // *** --- Pengaturan tombol Print --- *** //
                                $statusFakultas = $pivot->skripsi_fakultas->SKRIPSI_STATUS;
                                $statusPustaka = $pivot->skripsi_perpustakaan->SKRIPSI_STATUS;
                                $statusSoftcopy = $pivot->skripsi_softcopy->SKRIPSI_STATUS;

                                if (
                                    'Disetujui' == $statusFakultas &&
                                    'Disetujui' == $statusPustaka &&
                                    'Disetujui' == $statusSoftcopy
                                ) {
                                    $buttonPrint = null;
                                }
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-link"
                                        wire:click="detailPraja('{{ $item->SKRIPSI_PRAJA }}')" data-bs-toggle="modal"
                                ...
                                <td {{ $buttonClasses['keep'] }}>
                                    <button type="button"
                                        ...
                                        wire:click='keepData("{{ $item->SKRIPSI_ID }}")'>
                                ...
                                <td {{ $buttonClasses['approve'] }}>
                                    <button type="button"
                                        ...
                                        wire:click='approveData("{{ $item->SKRIPSI_ID }}")'>
                                ...
                                <td {{ $buttonPrint }}>
                                    <button type="button"
                                        ...
                                        wire:confirm='Cetak Pengajuan Similaritas {{ $item->SKRIPSI_PRAJA }} ?'
