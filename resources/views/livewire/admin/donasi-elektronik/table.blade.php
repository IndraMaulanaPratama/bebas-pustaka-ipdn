...
<div class="col-12">
    <div>
        <x-admin.components.card.card size=12 title="Data Pengajuan" titleSpan='Aktif'>
                        ...
                        @foreach ($elektronik as $item)
                            @php
                                $buttonClasses = $this->getButtonStatus(
                                    $item->ELEKTRONIK_STATUS,
                                    $item->ELEKTRONIK_OFFICER,
                                );
                            @endphp

                            <tr>
                                <td> {{ $loop->index + $elektronik->firstItem() }} </td>
                                ...
                                <td>
                                    <button type="button" class="btn btn-link"
                                        wire:click="detailPraja('{{ $item->ELEKTRONIK_PRAJA }}')" data-bs-toggle="modal"
                                        data-bs-target="#modalDetailPraja">
                                        {{ $item->ELEKTRONIK_PRAJA }}
                                    </button>
                                </td>

                                <td> {{ $item->ELEKTRONIK_ID_PO }} </td>
                                <td> {{ $item->ELEKTRONIK_NOTES }} </td>
                                <td> {{ $item->ELEKTRONIK_TANGGAL_PENGAJUAN ? $item->ELEKTRONIK_TANGGAL_PENGAJUAN->locale('id')->translatedFormat('d M Y H:i') : '-' }} </td>
                                <td> {{ $item->ELEKTRONIK_OFFICER === 1 ? null : $item->user->name }} </td>
                                <td> {{ $item->ELEKTRONIK_APPROVED ? \Carbon\Carbon::parse($item->ELEKTRONIK_APPROVED)->locale('id')->translatedFormat('d M Y H:i') : '-' }} </td>
                                ...
                                <td {{ $buttonClasses['keep'] }}>
                                    <button type="button"
                                        ...
                                        wire:click='keepData("{{ $item->ELEKTRONIK_ID }}")'>
                                ...
                                <td {{ $buttonClasses['approve'] }}>
                                    <button type="button"
                                        ...
                                        wire:click='approveData("{{ $item->ELEKTRONIK_ID }}")'>
                                ...
                                <td {{ $buttonClasses['print'] }}>
                                    <button type="button"
                                        ...
                                        wire:confirm='Cetak Bukti Donasi {{ $item->ELEKTRONIK_PRAJA }} ?'
            ...
            <x-admin.tamplates.paginate.paginate :item="$elektronik" />
